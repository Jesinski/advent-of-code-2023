...
export class MinHeap {
  ...
  private _percolateUp(index: number) {
    ...
    if (index <= 0) {
      ...
      this.heap[parent][LOOKUP_INDEX] > this.heap[index][LOOKUP_INDEX]
    ) {
  ...
  private _minHeapify(index: number) {
    ...
    if (
      this.elements > left &&
      this.heap[smallest][LOOKUP_INDEX] > this.heap[left][LOOKUP_INDEX]
    ) {
      smallest = left;
    }
    if (
      this.elements > right &&
      this.heap[smallest][LOOKUP_INDEX] > this.heap[right][LOOKUP_INDEX]
    ) {
      smallest = right;
    }
    if (smallest != index) {
